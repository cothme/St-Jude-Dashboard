...
import {
	deleteReplacedProfilePhoto,
	discardDraftProfilePhoto,
} from "../../shared/profilePhotos";
import {
	appointmentLogDetails,
	checkupLogDetails,
	employeeLogDetails,
	medicationAdministrationLogDetails,
	medicationScheduleLogDetails,
	patientDischargeLogDetails,
	patientLogDetails,
	payrollLogDetails,
	userLogDetails,
} from "../../shared/activityLogDetails";
import {
	doctorName,
	doctorNameFromEmployee,
	employeeName,
	patientName,
} from "../../shared/names";
import {
...
type EmployeeDraft = Omit<Employee, "id" | "employeeCode"> & {
	employeeCode?: string;
};
...
export function Employees() {
	...
	const [sort, setSort] = useState<
		SortState<
			"employee" | "position" | "department" | "salary" | "schedule" | "status"
		>
	...
	const closeEditor = () => {
		const previous =
			editing && "id" in editing
				? data.employees.find((employee) => employee.id === editing.id)
				: undefined;
		discardDraftProfilePhoto(
			editing?.profileImageKey,
			previous?.profileImageKey,
			showToast,
		);
		setEditing(null);
		setError("");
	...
	const removeEmployee = async (employee: Employee) => {
		if (
			!window.confirm(
				`Delete employee record for ${employee.firstName} ${employee.lastName}?`,
			)
		)
			return;
		setDeleteError("");
		...
		try {
			...
			logActivity({
				action: "Deleted",
				entity: "Employee",
				summary: `Deleted employee record for ${employee.firstName} ${employee.lastName}.`,
				details: employeeLogDetails(employee),
				severity: "danger",
			});
	...
	return (
		...
		>
						...
						<tbody>
							{sortedEmployees.map((employee) => (
								<tr
									...
									{...recordRowProps(
										() => setViewing(employee),
										`View employee details for ${employee.firstName} ${employee.lastName}`,
									)}
								>
									<td data-label="Employee">
										<div className="identity-cell">
											...
											<span>
												...
												<small>
													{employee.employeeCode} · {employee.sex}
												</small>
											</span>
										</div>
									</td>
									...
									<td data-label="Schedule">{employee.workDaysPerWeek}-day</td>
									...
									>
										<ActionIconButton
											label={`Edit ${employee.firstName} ${employee.lastName}`}
											icon={<Pencil size={16} />}
											onClick={(event) => {
										...
										<ActionIconButton
											variant="danger"
											label={`Delete ${employee.firstName} ${employee.lastName}`}
											icon={<Trash2 size={16} />}
			...
			{editing && (
				<Modal
					title={
						"id" in editing ? "Edit Employee Record" : "Add Employee Record"
					}
					onClose={closeEditor}
				>
			...
			{viewing && (
				<RecordDetailModal
					...
					items={[
						...
						{
							label: "Work schedule",
							value: `${viewing.workDaysPerWeek}-day schedule`,
						},
...
function EmployeeForm({
	...
	employee: Employee | EmployeeDraft;
	savedProfileImageKey?: string;
	isSaving?: boolean;
	onChange: (employee: Employee | EmployeeDraft) => void;
	onSubmit: (event: FormEvent<HTMLFormElement>) => void;
	onCancel: () => void;
}) {
	const set = (patch: Partial<EmployeeDraft>) =>
		onChange({ ...employee, ...patch });
	return (
		<form
			className="form-grid"
			onSubmit={onSubmit}
		>
			<ProfilePhotoField
				name={`${employee.firstName} ${employee.lastName}`}
				value={employee.profileImageUrl}
				fileKey={employee.profileImageKey}
				savedFileKey={savedProfileImageKey}
				onChange={(profileImageUrl, profileImageKey) =>
					set({ profileImageUrl, profileImageKey })
				}
			/>
			{"id" in employee && (
				<FormInput
			...
			<FormSelect
				label="Gender"
				value={employee.sex}
				onChange={(value) => set({ sex: value as Employee["sex"] })}
			>
				<option>Male</option>
			...
			>
				...
				<option>Nurse</option>
				<option>Nursing Attendant</option>
				<option>Cook</option>
			...
			<FormSelect
				label="Work schedule"
				value={employee.workDaysPerWeek}
				onChange={(value) => set({ workDaysPerWeek: Number(value) as 5 | 6 })}
			>
			...
			<FormSelect
				...
				onChange={(value) => set({ status: value as Employee["status"] })}
			>
